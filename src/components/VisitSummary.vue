  ...
  <div class="space-y-3">
    ...
    <div v-if="summary.staff || summary.service" class="bg-white rounded-3xl p-4 shadow-sm">
      ...
      <div v-if="summary.staff" class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          <div class="font-bold text-gray-900">{{ summary.staff.name }}</div>
          <div class="text-sm text-gray-500">{{ summary.staff.specialization }}</div>
        </div>
      </div>

      <!-- Gray Divider -->
      <div v-if="summary.staff && summary.service" class="border-t border-gray-200 my-4"></div>
      ...
      <div v-if="summary.service" class="grid grid-cols-[1fr_auto] gap-y-1.5 gap-x-3">
        ...
        <div class="font-bold text-gray-900">{{ summary.service.price.toLocaleString('ru-RU') }} ₽</div>
        <div class="text-gray-500 text-sm">{{ summary.service.duration }} мин</div>
        <div></div>
      </div>
    </div>

    <!-- Total Block -->
    <div v-if="summary.service" class="flex justify-between items-center bg-[#E7EDF9] rounded-xl p-3 font-bold">
      <span>Итого к оплате:</span>
      <span class="text-[#666FE8]">{{ summary.service.price.toLocaleString('ru-RU') }} ₽</span>
    </div>
  </div>
</template>

...
onMounted(async () => {
  ...
  // 1) Типы услуг
  const { data: types } = await api.get('/salon/services/types/')
  serviceTypes.value = types

  // 2) Все услуги
  const { data: all } = await api.get('/salon/services/')
...
function confirm() {
  ...
  // навигация на страницу подтверждения (имя можете поправить под ваш роутер)
  router.push({ path: '/appointment' })
}
</script>

<style scoped>
.services-view { max-width: 600px; margin: 2rem auto; padding: 1rem; background: #fff; border-radius: 8px; }
.loading { text-align: center; padding: 2rem; }
.type-block { border: 1px solid #eee; border-radius: 6px; margin-bottom: 1rem; overflow: hidden; }
.type-header { width: 100%; padding: .75rem 1rem; background: #f9f9f9; display: flex; justify-content: space-between; cursor: pointer; font-weight: bold; border: none; }
.service-list { list-style: none; margin: 0; padding: .5rem; background: #fff; }
.service-list li { padding: .5rem 0; display: flex; align-items: center; }
.service-list li + li { border-top: 1px solid #eee; }
.service-list label { width: 100%; display: flex; justify-content: space-between; cursor: pointer; }
.service-list .name { flex: 1; }
.service-list .price { margin-left: 1rem; }
.service-list li.selected { background: #e6f7ff; }
.btn-next { width: 100%; padding: .75rem; background: #007bff; color: #fff; border: none; border-radius: 6px; font-size: 1rem; margin-top: 1rem; cursor: pointer; }
